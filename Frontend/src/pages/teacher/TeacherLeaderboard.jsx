...
export default function TeacherLeaderboard() {
    ...
    const loadData = async () => {
      ...
      try {
        const [merit, social] = await Promise.all([
          fetchJsonCached(
            `${API_BASE_URL}/api/leaderboard/merit?timePeriod=${encodeURIComponent(
              timePeriod
            )}&competitionType=${encodeURIComponent(competitionType)}`,
            {
              token,
              ttlMs: 60000,
              cacheKey: `teacher:leaderboard:merit:${timePeriod}:${competitionType}`,
            }
          ),
          fetchJsonCached(
            `${API_BASE_URL}/api/leaderboard/social?timePeriod=${encodeURIComponent(
              timePeriod
            )}`,
            {
              token,
              ttlMs: 60000,
              cacheKey: `teacher:leaderboard:social:${timePeriod}`,
            }
          ),
        ]);
  ...
  useEffect(() => {
    if (activeTab !== "merit") setCompetitionType("all");
  }, [activeTab]);
      ...
      <div className="max-w-7xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="p-4 rounded-lg bg-info/5 border border-info/20">
          <p className="text-sm text-info">
            <strong>Teacher View:</strong> This is a read-only view of the leaderboards.
            {activeTab === "merit"
                  ...
                  <tr className="border-b border-border">
                    ...
                    {activeTab === "merit" ? (
                      <>
                        ...
                        <th className="text-center py-3 px-4 text-sm font-medium text-muted-foreground">
                          Achievements
                        </th><th className="text-center py-3 px-4 text-sm font-medium text-muted-foreground">
                          Participations
                        </th>
                      </>
                    ) : (
